...
function convertSRTtoJSON(f){
    if (!f){
        f = File.openDialog (prompt= "Choose an srt file", filter = "*.srt", multiSelect = false);
    }
    var subtitles = [];
    f.open("r");
    while (! f.eof){
        var timecodeLine = f.readln();
...
var f = File.openDialog (prompt= "Choose an srt file", filter = "*.srt", multiSelect = false);
var theJSON = convertSRTtoJSON (f);
jsonFile = File(f.toString().replace(".srt", ".json"));
jsonFile.open("w");
